...
2. For each pre-populated ad block:
   - Confirm `start:` and `end:` against the audio. SparkyPod's production ad markers are usually right but not perfect.
...
These rules mirror SparkyPod's ad-detection prompt, so ground truth and predictions are scored against the same expectation.
...
If SparkyPod's `create_windows` parameters change (window size, overlap), `windows.json` becomes stale. Regenerate per episode:
