...
# Channel-level podcast:* tags SparkyPod copies from the upstream feed unchanged.
...
# Channel-level podcast:* tags SparkyPod always removes. soundbite/liveItem/
...
# podcast:txt purpose values that SparkyPod refuses to forward.
# verify/applepodcastsverify are ownership tokens bound to the original
# publisher; ai-content is always re-asserted (true) by SparkyPod itself.
...
#   new-feed-url: would tell apps to redirect SparkyPod subscribers to the
...
# Standard RSS 2.0 channel tags SparkyPod carries through verbatim.
...
class RSSParser:
    ...
    def generate_episode_id(self, episode_url: str, guid: str = None) -> str:
        ...
        podcast-app subscription of every SparkyPod user -- a migration
    ...
    def modify_feed(self, feed_content: str, slug: str, storage=None,
        ...
        for entry in entries:
            ...
            # SparkyPod-served URL only when our regenerated file is
    ...
    def _append_podcasting2_tags(self, lines: list, slug: str, episode_id: str, storage) -> None:
        # Emit only when SparkyPod has the cached file. Upstream URLs must
    ...
    def build_combined_feed(self, episodes: List[Dict], storage=None,
                            channel_title: str = "SparkyPod — All Podcasts",
                            channel_link: Optional[str] = None,
                            channel_description: str = (
                                "All processed episodes from every podcast in SparkyPod, "
                                "newest first."
                            )) -> str:
        ...
        lines.append('<itunes:author>SparkyPod</itunes:author>')
    ...
    def _emit_channel_metadata_passthrough(self, lines: list, feed_content: str) -> None:
        ...
        lines.append('<generator>SparkyPod</generator>')
