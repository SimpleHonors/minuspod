...
def check_auth():
    ...
    if not password_hash or password_hash == '':
        # No app password configured: the operator has deliberately opted into a
        # fully-open instance (private Tailscale / LAN-only deployment). Allow
        # every request, including mutating and sensitive routes. Setting an app
        # password re-enables session auth + the CSRF check below.
        return None
...
from api import feeds, episodes, history, settings, system, patterns, sponsors, status, auth, search, podcast_search, stats, providers, tags, cue_templates, cue_detections, ad_inbox
