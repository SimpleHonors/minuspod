"""REST endpoints for per-feed audio cue templates (#350 v2).
...
- ``POST   /feeds/<slug>/episodes/<episode_id>/cue-template-preview``
                                                   - run a template against an
                                                     episode and return matches
"""
import logging
...
def _template_to_meta_dict(row: dict) -> dict:
    """Strip the binary blob for JSON responses."""
    return {
        'id': row['id'],
...
def create_cue_template(slug):
    ...
    from audio_analysis.cue_features import (
        SAMPLE_RATE_HZ, N_COEFFS, compute_mfcc, decode_pcm_window, serialize_mfcc,
    )
    try:
