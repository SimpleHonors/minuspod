...
# dynaudnorm presets for the optional second-pass loudness flattener.
# Smaller frame (f) reacts to shorter loud spikes (more level-bouncing);
# larger frame is gentler. Higher peak target (p) is louder-but-safe.
NORMALIZE_PRESETS = {
...
class AudioProcessor:
    ...
    def normalize_audio(self, input_path: str,
        ...
        This is intentionally a SEPARATE invocation from remove_ads —
        fusing dynaudnorm into the cut filter graph would risk the cut
        behavior across the variety of podcast feeds. Cost: ~3-5s on a
        48-min episode.
        """
